...
Use for changes that span multiple modules, affect shared packages, change workspace behavior, introduce new data flow, or need reviewable architecture. Skip or keep very short for single-file UI fixes.
...
4. **Detailed plan** — concrete implementation steps, types/APIs/state introduced, migrations or config changes, UI primitives reused, and tradeoffs. Explain why the design fits Sudomd's existing patterns.
...
- **End-to-end flow** — include when tracing across UI, editor, or runtime clarifies the design.
